#[doc = "🦃"]
pub const TURKEY: crate::Emoji = crate::Emoji {
    glyph: "🦃",
    codepoint: "1F983",
    status: crate::Status::FullyQualified,
    introduction_version: 1f32,
    name: "turkey",
    annotations: &["bird", "turkey"],
    variants: &[],
};
#[doc = "🐔"]
pub const CHICKEN: crate::Emoji = crate::Emoji {
    glyph: "🐔",
    codepoint: "1F414",
    status: crate::Status::FullyQualified,
    introduction_version: 0.6f32,
    name: "chicken",
    annotations: &["bird", "chicken"],
    variants: &[],
};
#[doc = "🐓"]
pub const ROOSTER: crate::Emoji = crate::Emoji {
    glyph: "🐓",
    codepoint: "1F413",
    status: crate::Status::FullyQualified,
    introduction_version: 1f32,
    name: "rooster",
    annotations: &["bird", "rooster"],
    variants: &[],
};
#[doc = "🐣"]
pub const HATCHING_CHICK: crate::Emoji = crate::Emoji {
    glyph: "🐣",
    codepoint: "1F423",
    status: crate::Status::FullyQualified,
    introduction_version: 0.6f32,
    name: "hatching chick",
    annotations: &["baby", "bird", "chick", "hatching", "hatching chick"],
    variants: &[],
};
#[doc = "🐤"]
pub const BABY_CHICK: crate::Emoji = crate::Emoji {
    glyph: "🐤",
    codepoint: "1F424",
    status: crate::Status::FullyQualified,
    introduction_version: 0.6f32,
    name: "baby chick",
    annotations: &["baby", "baby chick", "bird", "chick"],
    variants: &[],
};
#[doc = "🐥"]
pub const FRONT_FACING_BABY_CHICK: crate::Emoji = crate::Emoji {
    glyph: "🐥",
    codepoint: "1F425",
    status: crate::Status::FullyQualified,
    introduction_version: 0.6f32,
    name: "front-facing baby chick",
    annotations: &["baby", "bird", "chick", "front-facing baby chick"],
    variants: &[],
};
#[doc = "🐦"]
pub const BIRD: crate::Emoji = crate::Emoji {
    glyph: "🐦",
    codepoint: "1F426",
    status: crate::Status::FullyQualified,
    introduction_version: 0.6f32,
    name: "bird",
    annotations: &["bird"],
    variants: &[],
};
#[doc = "🐧"]
pub const PENGUIN: crate::Emoji = crate::Emoji {
    glyph: "🐧",
    codepoint: "1F427",
    status: crate::Status::FullyQualified,
    introduction_version: 0.6f32,
    name: "penguin",
    annotations: &["bird", "penguin"],
    variants: &[],
};
#[doc = "🕊\u{fe0f}"]
pub const DOVE: crate::Emoji = crate::Emoji {
    glyph: "🕊\u{fe0f}",
    codepoint: "1F54A FE0F",
    status: crate::Status::FullyQualified,
    introduction_version: 0.7f32,
    name: "dove",
    annotations: &["bird", "dove", "fly", "peace"],
    variants: &[crate::Emoji {
        glyph: "🕊",
        codepoint: "1F54A",
        status: crate::Status::Unqualified,
        introduction_version: 0.7f32,
        name: "dove",
        annotations: &[],
        variants: &[],
    }],
};
#[doc = "🦅"]
pub const EAGLE: crate::Emoji = crate::Emoji {
    glyph: "🦅",
    codepoint: "1F985",
    status: crate::Status::FullyQualified,
    introduction_version: 3f32,
    name: "eagle",
    annotations: &["bird", "eagle"],
    variants: &[],
};
#[doc = "🦆"]
pub const DUCK: crate::Emoji = crate::Emoji {
    glyph: "🦆",
    codepoint: "1F986",
    status: crate::Status::FullyQualified,
    introduction_version: 3f32,
    name: "duck",
    annotations: &["bird", "duck"],
    variants: &[],
};
#[doc = "🦢"]
pub const SWAN: crate::Emoji = crate::Emoji {
    glyph: "🦢",
    codepoint: "1F9A2",
    status: crate::Status::FullyQualified,
    introduction_version: 11f32,
    name: "swan",
    annotations: &["bird", "cygnet", "swan", "ugly duckling"],
    variants: &[],
};
#[doc = "🦉"]
pub const OWL: crate::Emoji = crate::Emoji {
    glyph: "🦉",
    codepoint: "1F989",
    status: crate::Status::FullyQualified,
    introduction_version: 3f32,
    name: "owl",
    annotations: &["bird", "owl", "wise"],
    variants: &[],
};
#[doc = "🦤"]
pub const DODO: crate::Emoji = crate::Emoji {
    glyph: "🦤",
    codepoint: "1F9A4",
    status: crate::Status::FullyQualified,
    introduction_version: 13f32,
    name: "dodo",
    annotations: &["Mauritius", "dodo", "extinction", "large"],
    variants: &[],
};
#[doc = "🪶"]
pub const FEATHER: crate::Emoji = crate::Emoji {
    glyph: "🪶",
    codepoint: "1FAB6",
    status: crate::Status::FullyQualified,
    introduction_version: 13f32,
    name: "feather",
    annotations: &["bird", "feather", "flight", "light", "plumage"],
    variants: &[],
};
#[doc = "🦩"]
pub const FLAMINGO: crate::Emoji = crate::Emoji {
    glyph: "🦩",
    codepoint: "1F9A9",
    status: crate::Status::FullyQualified,
    introduction_version: 12f32,
    name: "flamingo",
    annotations: &["flamboyant", "flamingo", "tropical"],
    variants: &[],
};
#[doc = "🦚"]
pub const PEACOCK: crate::Emoji = crate::Emoji {
    glyph: "🦚",
    codepoint: "1F99A",
    status: crate::Status::FullyQualified,
    introduction_version: 11f32,
    name: "peacock",
    annotations: &["bird", "ostentatious", "peacock", "peahen", "proud"],
    variants: &[],
};
#[doc = "🦜"]
pub const PARROT: crate::Emoji = crate::Emoji {
    glyph: "🦜",
    codepoint: "1F99C",
    status: crate::Status::FullyQualified,
    introduction_version: 11f32,
    name: "parrot",
    annotations: &["bird", "parrot", "pirate", "talk"],
    variants: &[],
};
